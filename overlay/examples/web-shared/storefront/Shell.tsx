...
export function StoreShell<V extends string>({
  ...
  /** Optional navigation-only behavior for the first view; internal agent handoffs are unaffected. */
  onHomeClick?: () => void;
  chat: AgentTurn;
  api: AgentApi;
            ...
            {views.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => (item.id === home && onHomeClick ? onHomeClick() : onViewChange(item.id))}
