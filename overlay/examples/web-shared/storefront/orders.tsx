...
function OrderRow({
  ...
  const estimate = estimateOf(order);
  const deliveryLabel = ["shipped", "out_for_delivery", "delayed"].includes(order.status) ? "Track" : "Details";
  return (
    <li className="flex items-center gap-3 border-t border-(--line) px-[18px] py-3 first:border-t-0">
      ...
      {!compact && onOpenOrder ? (
        <button
          ...
          aria-label={`${deliveryLabel} order ${order.order_id}`}
          className="rounded-[4px] border border-(--line-strong) bg-(--card) px-3 py-2 text-[12px] font-semibold text-(--ink) transition hover:border-(--accent-strong) hover:bg-(--accent-soft)"
        >
          {deliveryLabel}
        </button>
      ) : null}
    ...
    <StorePage>
      ...
      {orders === null ? (
        ...
        <Panel>
          <ul>
            {shown.map((order) => (
              <OrderRow key={order.order_id} order={order} nouns={nouns} thumb={thumb} onOpenOrder={onOpenOrder} />
            ))}
          </ul>
        </Panel>
      ) : (
