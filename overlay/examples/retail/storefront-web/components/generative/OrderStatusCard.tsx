...
import { openOrderTracking } from "@/lib/order-tracking";
...
export default function OrderStatusCard({ payload }: { payload: OrderStatusPayload }) {
  ...
  const nextStep = payload.next_step && !/\b(track|carrier|real-time)\b/i.test(payload.next_step)
    ? payload.next_step
    : null;
  return (
    <section className="rounded-2xl border border-(--line) bg-(--card) p-4 shadow-(--shadow-sm)">
      ...
      {nextStep ? <p className="mt-2 text-[15px] font-medium text-(--ink)">{nextStep}</p> : null}
    </section>
  );
}
